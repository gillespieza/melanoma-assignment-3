...
ROOT_DIR = DATA_DIR.parent  # real project root (for display-only relative paths)
...
def download_and_extract_cbioportal_dataset(study_id: str, target_dir: Path) -> None:
    ...
    if patient_file.exists() and patient_file.stat().st_size > 0:
        print(f"Dataset {study_id} already exists in {target_dir.relative_to(ROOT_DIR).as_posix()} and is non-empty.")
        return

    if target_dir.exists():
        try:
            shutil.rmtree(target_dir, ignore_errors=False)
            print(f"Removed existing directory {target_dir.relative_to(ROOT_DIR).as_posix()} to ensure a clean extraction.")
        except Exception as e:
            print(f"Failed to remove existing directory {target_dir.relative_to(ROOT_DIR).as_posix()}: {e}")
            raise
    ...
    try:
        ...
        if extracted_dir.exists() and extracted_dir.is_dir():
            print(f"Reorganising files from {study_id} to {target_dir.relative_to(ROOT_DIR).as_posix()}...")
...
if __name__ == "__main__":
    ...
    with open(LOG_PATH, "w", encoding="utf-8") as log_file:
        ...
        with contextlib.redirect_stdout(stdout_tee), contextlib.redirect_stderr(stderr_tee):
            print(f"Logging console output to {LOG_PATH.relative_to(ROOT_DIR).as_posix()}")
            main()
