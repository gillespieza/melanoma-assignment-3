...
def clean_iatlas_cohort(
    ...
    if not all((raw_dir / f).exists() for f in required):
        raise FileNotFoundError(
            f"Missing raw input files in {raw_dir.relative_to(ROOT_DIR).as_posix()}. Please run download_data.py first."
        )
...
def clean_tcga_skcm() -> None:
    ...
    if mut_path.exists():
        ...
        if not df_mut.empty:
            ...
            df_mut_final.index.name = "SAMPLE_ID"
            df_mut_final.to_csv(proc_dir / "mutations_cleaned.csv")
            print(f"  TCGA-SKCM: Cleaned mutations written to {(proc_dir / 'mutations_cleaned.csv').relative_to(ROOT_DIR).as_posix()}")
