...
def main() -> None:
    ...
    doc_sections.append(
        ...
        f"> - **Scope**: `IMPRES` is **present in `feature_matrix.csv`** (the stored {n_bio_features}-feature multi-modal data object) and is a candidate predictor in **Phase 5** (Subgroup Predictive Modelling) and **Phase 6** (Clinical Utility Analysis). It is **not** part of the GMM clustering core (which uses only the 6 continuous immune/stromal features: `TIS`, `CYT`, `CD8_T_cells`, `M1_Macrophages`, `M2_Macrophages`, `CAFs`) and is not part of the TME deconvolution core (Macrophage STV, cell-type fractions, or spatial proxy indicators) because it encodes a discrete checkpoint pairwise logic rather than a continuous microenvironment abundance estimate.\n"
        f"> - **Feature Matrix vs. Model Inputs**: `feature_matrix.csv` stores the full {n_bio_features}-column multi-modal dataset (including genomic metadata such as neoantigen burden columns) for reference and downstream analysis. The GMM (Phase 3) uses only 6 features; the Phase 5 subgroup supervised models draw from a 19-candidate panel (`CANDIDATE_PREDICTION_FEATURES` in `05_subgroup_models.py`), using up to 13 features per phenotype after excluding the 6 GMM clustering features.\n"
