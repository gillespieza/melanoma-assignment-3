...
def _process_raw_expression_matrix(
    ...
    df_expr.index.name = _COL_SAMPLE_ID
    try:
        df_expr = df_expr.astype(np.float32)
...
def _extract_raw_non_synonymous_genes(
    ...
    try:
        header_df = pd.read_csv(mutation_path, sep="\t", comment="#", nrows=0)
        if not _validate_raw_maf_columns(header_df):
            return None
        target_cols = [_COL_HUGO_SYMBOL, _COL_VARIANT_CLASSIFICATION]
        df_raw = pd.read_csv(
            mutation_path,
            sep="\t",
            comment="#",
