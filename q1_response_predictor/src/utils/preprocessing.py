...
def parse_maf_mutations(
    ...
    header_df = pd.read_csv(maf_path, sep="\t", comment="#", nrows=0)
    ...
    missing_columns = required_columns.difference(all_cols)
    if missing_columns:
        raise ValueError(
    ...
    needed_cols = set(required_columns)
    for opt_col in ("HGVSp_Short", "HGVSp", "Protein_Change"):
        if opt_col in all_cols:
            needed_cols.add(opt_col)
