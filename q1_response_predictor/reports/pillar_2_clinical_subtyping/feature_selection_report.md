...
This report presents a **two-tiered feature selection architecture** evaluating clinical and transcriptomic predictors of **immunotherapy binary response (RECIST)** across the 6 active immunotherapy datasets (Liu 2019, Hugo 2016, Riaz 2017, TCGA GDC 2025, Gide 2019, and Van Allen 2015). The dataset pool utilizes the updated NCI Genomic Data Commons TCGA-SKCM cohort (`skcm_tcga_gdc`), which supersedes the legacy 2018 TCGA Pan-Cancer Atlas dataset due to updated GDC data harmonization. Feature selection is conducted strictly on the immunotherapy-treated (IT-treated) subcohort with RECIST response labels (anti-PD-1, anti-CTLA-4, and vaccine arms where applicable), excluding non-IT-treated patients such as surgery-only or chemotherapy-only cases.

- **Tier 1 (ICI Multi-Cohort Immune & Genomic Signatures, $N = 473$)**: Evaluates 18 features across 5 biological domains — 6 transcriptomic immune signatures (`IFN_gamma`, `TIS`, `CYT`, `CD8_Tcell`, `IMPRES`, `PD_L1`), 2 myeloid signatures (`M1_M2_Ratio`, `Macrophage_STV`), 3 genomic burden metrics (`TMB`, `SNV_NEOANTIGEN`, `INDEL_NEOANTIGEN`), 4 driver mutation subtypes (`mut_BRAF_V600`, `mut_BRAF_nonV600`, `mut_NRAS`, `mut_NF1`), and 3 host demographics (`AGE`, `SEX_Male`, `SEX_Female`) — pooled across **Liu 2019**, **Hugo 2016**, **Riaz 2017**, **TCGA GDC 2025**, **Gide 2019**, and **Van Allen 2015**.
- **Tier 2 (ICI Granular Clinical Features, $N = 473$)**: Evaluates 27 granular categorical baseline clinical covariates available within the ICI trial cohorts.

## 1. Tier 1: ICI Multi-Cohort _Immune_ Feature Selection ($N = 473$)

> [!INFO] What, Why & Questions — Tier 1
> - **What**: Evaluating 18 features across 5 biological domains (6 transcriptomic immune signatures, 2 myeloid signatures, 3 genomic burden metrics, 4 driver mutation subtypes, and 3 host demographics) against **immunotherapy binary response** ($N_{\text{resp}} = 201$ / $N_{\text{non-resp}} = 272$) across all 6 ICI cohorts pooled ($N = 473$). Two complementary models are applied: **Random Forest** importance and **Logistic Regression**.
...
- **Total ICI Sample Size**: 473 patients across 6 cohorts (Liu 2019, Hugo 2016, Riaz 2017, TCGA GDC 2025, Gide 2019, and Van Allen 2015)
...
| **Genomic Driver Subtypes** | `mut_BRAF_V600`, `mut_BRAF_nonV600`, `mut_NRAS`, `mut_NF1` | **Oncogenic Secretome & Kinase Activation**: `BRAF V600` (Class 1 hotspots V600E/K) drives monomeric MAPK hyperactivation and MHC-I downregulation; `BRAF Non-V600` (Class 2/3) relies on RAS dimerization. `NRAS`/`NF1` mutations associate with high UV mutational burden. |
...
> - **Top Predictor**: **`Tumour Inflammation Signature (TIS) (Z-Score)`** accounts for 10.2% of total RF Gini importance across all features.
> - **Top 5 Features by Gini Importance**:
>   - `Tumour Inflammation Signature (TIS) (Z-Score)` — 10.2%
>   - `CD8 T-Cell Abundance Signature (Z-Score)` — 9.9%
>   - `Cytolytic Activity (CYT) Score (Z-Score)` — 9.8%
>   - `Interferon-Gamma (IFN-γ) 6-Gene Signature (Z-Score)` — 9.7%
...
> - **Top Nominal Candidate**: **`Immune Predictive Score (IMPRES) (Z-Score)`** demonstrates the strongest unadjusted trend ($\text{aOR} = 1.35$, nominal $p = 0.014$), though it does not clear FDR correction (FDR $= 0.218$). High-dimensional modelling (Pillar 3/4) is required to aggregate these weak, correlated signals.
...
- **ICI Cohort Sample Size**: 473 patients (Liu 2019, Hugo 2016, Riaz 2017, TCGA GDC 2025, Gide 2019, and Van Allen 2015)
- **Response Evaluation Cohort**: 473 patients (201 responders / 272 non-responders)
...
> - **Top Overall Biomarker**: **`Cytolytic Activity (CYT) Score (Z-Score)`** ranks #1 across all molecular and clinical variables, accounting for 10.4% of total Gini importance ($N = 473$).
> - **Top 5 Features Across Domains**:
>   - `Cytolytic Activity (CYT) Score (Z-Score)` — 10.4%
>   - `Tumour Inflammation Signature (TIS) (Z-Score)` — 9.5%
>   - `Interferon-Gamma (IFN-γ) 6-Gene Signature (Z-Score)` — 9.3%
>   - `CD8 T-Cell Abundance Signature (Z-Score)` — 8.9%
>   - `PD-L1 Transcript Proxy (Z-Score)` — 7.6%
...
> 1. **Tier 1 Top Response Biomarker**: **`Immune Predictive Score (IMPRES) (Z-Score)`** is the single strongest univariate predictor of response across all 473 ICI patients ($\text{OR} = 1.50$, $p = 6.60 \times 10^{-5}$, FDR $= 0.001$).
> 2. **Tier 1 Independent Biomarker**: In joint multivariate modelling ($N = 473$), **`Immune Predictive Score (IMPRES) (Z-Score)`** remains an independent predictor ($\text{aOR} = 1.35$, $p = 0.014$, FDR $= 0.218$; Model AUC = **0.660**).
...
> 4. **Biological Alignment**: Microenvironmental T-cell inflammation signatures consistently associate with response ($\text{OR} > 1.0$) across all 473 ICI patients.
...
> - **Cohort Heterogeneity**: Liu 2019, Hugo 2016, Riaz 2017, TCGA GDC 2025, Gide 2019, and Van Allen 2015 differ in treatment agent, response definition, and biopsy timing.
...
>
> - [`run_clinical_feature_selection.py`](../../scripts/pillar_2_clinical_subtyping/run_clinical_feature_selection.py): Evaluates immune signatures (Tier 1) & clinical covariates (Tier 2) via RF/Logit.
> - [`run_univariate_associations.py`](../../scripts/pillar_2_clinical_subtyping/run_univariate_associations.py): Evaluates per-cohort and pooled univariate statistical associations & forest plots.
> - [`clean_data.py`](../../scripts/pillar_1_cohort_preprocessing/clean_data.py): Preprocesses raw clinical metadata and expression into cleaned CSV matrices.
