...
survival. Two mechanistic readouts are examined:

  * pERK          — proliferative drive from the MAPK cascade
  * tumour burden — integrative readout (proliferation vs. immune control)

For each readout:
  A. Loop over the 10 vemurafenib-dose columns, fit a Cox PH model to each,
     and pick the dose with the smallest p-value ("optimal dose").
...
Output:
    plots/km_ode_stratified.pdf           (combined KM figure — both readouts)
    plots/cox_dose_scan.pdf               (Cox p-value vs dose, both readouts)
    plots/km_perk.png, plots/km_tumour_burden.png
...
def run_cox_scan(sim_df, clinical_df, label, ax=None):
    """
    Fit a univariate Cox PH model at every vemurafenib-dose column and return
    the dose giving the smallest p-value plus the merged analysis frame.
    Optionally draw the p-value-vs-dose curve onto `ax`.
    """
    dose_cols = [c for c in sim_df.columns if c.startswith("BRAFi_")]
...
print(f"    Clinical: {len(clinical)}  |  pERK sims: {perk_sim.shape}  "
      f"|  tumour sims: {tumour_sim.shape}")

readouts = [("pERK", perk_sim), ("tumour burden", tumour_sim)]

# Cox dose scan figure
fig_scan, axes_scan = plt.subplots(1, 2, figsize=(13, 4.5))
# KM figure
fig_km, axes_km = plt.subplots(1, 2, figsize=(14, 6))

summary = ["ODE Model — Survival Analysis Summary (TCGA-SKCM Melanoma)",
           "=" * 60]

for j, (label, sim) in enumerate(readouts):
    print("\n" + "-" * 55)
    print(f"  Analysing ODE readout: {label}")
    print("-" * 55)
    opt_col, cox_p, merged = run_cox_scan(sim, clinical, label, ax=axes_scan[j])
    thr, lr_p, n_hi, n_lo, direction, med_hi, med_lo = optimal_threshold_km(
        merged, opt_col, label, ax=axes_km[j])
    summary += [
        f"\n{label}:",
        f"  Optimal vemurafenib dose : {opt_col}",
...
fig_scan.suptitle("Cox PH p-value vs vemurafenib dose — TCGA-SKCM", fontweight="bold")
...
for (label, _), ax in zip(readouts, axes_km):
...
summary_txt = "\n".join(summary) + "\n\nInterpretation:\n" + (
    ...
    "   * Modelled tumour burden integrates proliferation against immune control;\n"
    "     high residual burden marks aggressive, immune-cold disease.\n"
    "  Both readouts are prognostic.\n")
