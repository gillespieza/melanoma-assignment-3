...
>
> **Last updated**: 2026-08-01 (Phase 6 refactoring & architecture sync: completed 3-pass code smell audit of 06_clinical_utility.py, standardising compute_net_benefit return tuple, decomposing functions to <= 30 lines, integrating safe_save_csv, centralising DCA constants, and adding zero-division and single-class safeguards.)
...
| `q5/scripts/05_subgroup_models.py` — code smells & calibration degeneracy (Phase 5 audit 2026-08-01) | Initial, second & fourth pass refactoring completed: (1) Removed orphaned partial `train_and_eval_loco` header. (2) Streamlined all overlength functions (`evaluate_predictions`, `_fit_single_phenotype_production_model`, `_fit_production_models`) to $\le 20$ lines (100% of functions in module are strictly $\le 30$ lines). (3) Refined `_fit_calibrator` return type hint to `Union[_IdentityPredictor, _LRPredictor, IsotonicRegression]`. (4) Added zero-variance calibration guard `if np.std(raw_cal_prob) < 1e-6: return _IdentityPredictor()` to prevent logistic regression fitting on constant prediction arrays. | **Resolved** (2026-08-01: zero overlength functions, robust calibration) |
