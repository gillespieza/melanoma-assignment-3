...
| Report Intro Rationale Correction (TCGA GDC IT-Treated Subcohort) | Corrected introduction text in `_build_report_intro_paragraph()` in `run_clinical_feature_selection.py` and `feature_selection_report.md` to accurately state that `skcm_tcga_gdc` is included as the updated 2025 NCI Genomic Data Commons cohort (superseding legacy 2018 TCGA PanCan Atlas due to updated GDC data harmonization) and that feature selection is conducted strictly on its IT-treated subcohort with RECIST response labels ($N = 41$). Verified 100% functions $\le 30$ lines in AST, 0 lines $> 100$ chars, and clean execution exit code 0. | **Resolved** (2026-08-09) |
