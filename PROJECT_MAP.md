...
| `run_univariate_associations.py` code smell remediation | Refactored `q1-response-predictor/scripts/pillar-2-clinical-subtyping/run_univariate_associations.py` per `AGENTS.md` guidelines: fixed subproject log routing (`get_subproject_log_dir`), removed 5 unused imports, extracted 6 domain constants (`_SIGNIFICANCE_ALPHA`, `_CI_LOWER_DISPLAY`, `_CI_UPPER_DISPLAY`, `_PLOT_FIGSIZE`, `_POOLED_COLS`), decomposed 100% of functions to $\le 30$ lines in AST, zero lines $>100$ characters, 100% docstring & type hint coverage, eliminated in-place DataFrame mutations in loops, and verified execution with exit code 0. | **Resolved** (2026-08-07) |
