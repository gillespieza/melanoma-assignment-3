...
>
> **Last updated**: 2026-08-07 (Completed code smell remediation for `q1-response-predictor/scripts/pillar-2-clinical-subtyping/plot_cluster_profile_visualizations.py`: added CSV export to `run_clinical_clustering.py` saving `data/processed/merged/clinical_clusters.csv`, extracted reusable `build_radar_angles()` helper to `src/utils/plotting.py`, replaced all hardcoded magic radar values and N-counts with dynamic DataFrame calculations, aligned palette to `PHENOTYPE_PALETTE`, fixed `get_subproject_log_dir` log routing, decomposed functions to $\le 30$ lines, fixed pre-existing `CONFIG_DIR` import error in `run_clinical_clustering.py`, and verified clean execution).
