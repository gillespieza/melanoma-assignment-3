...
>
> **Last updated**: 2026-08-01 (Phase 7 second-pass refactoring: DRY helpers `_safe_z_score`, `_safe_minmax`, `_add_phenotype_column`, `_build_alignment_scores` eliminate 13 repeated column-guard and map idioms; removed no-op fallback path in `load_q2_dabrafenib_weights()`; centralised 12 additional residual magic numbers; renamed `ax1`/`ax2` params to `ax`. 32 functions total, all ≤ 30 lines.)
