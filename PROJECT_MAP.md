...
| `q1-response-predictor/src/models.py` sklearn 1.8 `penalty` deprecation | scikit-learn 1.8 deprecated `penalty='l1'`, `penalty='l2'`, and `penalty='elasticnet'` on `LogisticRegression`, scheduling removal in 1.10. This generated `FutureWarning` and `UserWarning` spam across every `GridSearchCV` candidate during 5-fold CV and LOCO runs. Fixes: (1) `tune_logistic_regression` param grid changed from `penalty: ['l1', 'l2']` to `l1_ratio: [0.0, 1.0]` (0.0 = L2, 1.0 = L1); (2) `tune_elasticnet` and `get_uncalibrated_model("elasticnet")` had `penalty='elasticnet'` removed — `solver='saga'` with `l1_ratio` in `(0, 1)` implicitly selects ElasticNet; (3) `calibrate_estimator` now calls `np.asarray()` on inputs before `CalibratedClassifierCV.fit()` to suppress `UserWarning: X does not have valid feature names` triggered when the calibrator's internal CV folds receive numpy array slices of a DataFrame-trained estimator; (4) `generate_5f_cv_heatmap.py` wraps `X_val_scaled` in `pd.DataFrame(..., columns=FEATURE_COLS)` for feature-name consistency at predict time. | **Resolved** (2026-08-09) |
