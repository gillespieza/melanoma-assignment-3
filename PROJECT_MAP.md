...
| `q5/scripts/05_subgroup_models.py` — code smells (Phase 5 audit 2026-08-01) | Decomposed all functions exceeding 30 lines (`train_and_eval_loco`, `_fit_production_models`, `_compute_subgroup_oof_predictions`, `get_feature_columns`, `_evaluate_cluster_subgroups`, `main()`) into clean, single-responsibility sub-helpers (`_extract_phenotype_probabilities`, `_fit_single_phenotype_production_model`, `_predict_subgroup_fold`, `_evaluate_single_cluster_subgroup`, `_serialize_production_models`, `_generate_phase5_plots`). Replaced magic numbers with module constants (`CANDIDATE_PREDICTION_FEATURES`, `MIN_EFFECTIVE_SAMPLES`, `TOP_N_IMPORTANCES`, `GRID_LINE_COLOR`, `PLATT_C`, `PLATT_SOLVER`, `PLATT_MAX_ITER`). Replaced direct dict lookups with `get_phenotype_color()`. Added defensive `np.nan_to_num(..., nan=0.0)` zero-variance scaling protection in full feature matrix preprocessing. All functions strictly <= 30 lines. | **Resolved** (2026-08-01) |
