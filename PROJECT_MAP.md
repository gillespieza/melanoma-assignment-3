...
| `q5/scripts/05_subgroup_models.py` — single-class calibration & OOF prediction fallbacks | Resolved single-class calibration failure via `_IdentityPredictor` fallback and guards (`len(y_eff)<2`, `len(np.unique(y_cal))<2`, `np.isnan(raw_cal_prob)`). Resolved degenerate zero-weight OOF predictions by pre-filtering training features/labels to non-zero weight samples (`w_tr_k > MIN_PROB_WEIGHT`) in `_build_subgroup_k_pred`, eliminating Scikit-Learn tree bootstrap division-by-zero (`0 NaNs generated`). | **Resolved** (2026-08-01: zero NaNs in LOCO CV) |
...
| `q5/scripts/05_subgroup_models.py` — code smells (Phase 5 audit 2026-08-01) | Initial, second & third pass refactoring completed: (1) Decomposed all functions exceeding 30 lines (`train_and_eval_loco`, `_fit_production_models`, `_compute_subgroup_oof_predictions`, `get_feature_columns`, `_evaluate_cluster_subgroups`, `main()`) into clean sub-helpers (`_build_subgroup_k_pred`, `_build_cluster_subgroup_results`, `_build_overall_cohort_results`, `_assign_oof_predictions`, `_draw_roc_curves`, `_prepare_performance_melt_df`, `_build_importance_dataframe`, `_load_and_validate_clusters`). All functions are strictly <= 25 lines. (2) Added defensive `OUTPUT_DIR.mkdir(parents=True, exist_ok=True)` guards prior to CSV saves. (3) Replaced magic numbers with module constants (`CANDIDATE_PREDICTION_FEATURES`, `MIN_EFFECTIVE_SAMPLES`, `TOP_N_IMPORTANCES`, `GRID_LINE_COLOR`, `PLATT_C`, `PLATT_SOLVER`, `PLATT_MAX_ITER`). (4) Standardized visual color lookups via `get_phenotype_color()`. (5) Added `np.nan_to_num(..., nan=0.0)` zero-variance scaling protection. (6) Enhanced module docstrings with explicit output manifests and refined calibrator type annotations (`Union[_IdentityPredictor, _LRPredictor, IsotonicRegression]`). | **Resolved** (2026-08-01) |
