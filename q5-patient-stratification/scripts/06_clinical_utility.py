...
class _CalibratedModel:
    ...
    calibrator: Union[_LRPredictor, object]

    def __init__(self, base_clf: RandomForestClassifier, calibrator: Union[_LRPredictor, object]) -> None:
...
from src.utils.paths import PROCESSED_DIR, rel_path
...
DEFAULT_DCA_THRESHOLDS = np.linspace(0.01, 0.99, 99)
CLINICAL_DECISION_WINDOW = (0.20, 0.50)
TARGET_THRESHOLDS = [0.20, 0.30, 0.40]
FIXED_PT_THRESHOLD = 0.30
DCA_XLIM = (0.05, 0.80)
DCA_YMIN = -0.05
...
def get_feature_columns(df: pd.DataFrame) -> List[str]:
    ...
    return [
        col for col in CANDIDATE_PREDICTION_FEATURES
...
def compute_net_benefit(
    ...
    """Compute Net Benefit, TP, FP, TN, and FN at threshold pt."""
    N = len(y_true)
    if N == 0:
...
def _compute_dca_rows_for_threshold(
    y_true: np.ndarray, probability_cols: Dict[str, np.ndarray], pt: float
) -> List[Dict[str, Union[float, str, int]]]:
    """Compute DCA strategy metrics for a single threshold probability pt."""
    N = len(y_true)
    weight = pt / (1.0 - pt) if pt < 1.0 else np.nan
    tp_all = np.sum(y_true == 1)
    fp_all = np.sum(y_true == 0)
    nb_all = (tp_all / N) - (fp_all / N) * weight if pd.notna(weight) else 0.0
    ppv_all = tp_all / (tp_all + fp_all) if (tp_all + fp_all) > 0 else 0.0
    nnt_all = 1.0 / ppv_all if ppv_all > 0 else np.nan

    rows: List[Dict[str, Union[float, str, int]]] = [
        {
            "Threshold": pt,
            "Strategy": "Treat None",
            "Net_Benefit": 0.0,
            ...
            "PPV": np.nan,
            "NNT": np.nan,
            "Unnecessary_Treatments_Avoided": int(fp_all),
        },
        {
            "Threshold": pt,
            "Strategy": "Treat All",
...
def calculate_dca_curves(
    ...
    all_rows: List[Dict[str, Union[float, str, int]]] = []
    for pt in thresholds:
        all_rows.extend(_compute_dca_rows_for_threshold(y_true, probability_cols, pt))
...
def _predict_single_gene_benchmarks(
    ...
    if "PD_L1" in df.columns and df["PD_L1"].notna().any() and has_two_classes:
        ...
        pred_pdl1 = np.full(len(df), np.mean(y_true) if len(y_true) > 0 else 0.0)

    if "TMB_NONSYNONYMOUS" in df.columns and df["TMB_NONSYNONYMOUS"].notna().any() and has_two_classes:
        ...
        pred_tmb = np.full(len(df), np.mean(y_true) if len(y_true) > 0 else 0.0)

    return pred_pdl1, pred_tmb


def _predict_stratified_probabilities(
    df: pd.DataFrame, X_scaled: np.ndarray, pred_global: np.ndarray
) -> np.ndarray:
    """Combine phenotype-specific model predictions using soft GMM membership probabilities."""
...
def generate_predictions(df: pd.DataFrame) -> Dict[str, np.ndarray]:
    ...
    pred_stratified = _predict_stratified_probabilities(df, X_scaled, pred_global)
...
def _style_dca_axes(ax: plt.Axes, max_nb: float) -> None:
    """Apply titles, labels, limits, and annotations to DCA axes."""
    ax.set_ylim([DCA_YMIN, max(0.40, max_nb * 1.1)])
    ax.set_xlim(DCA_XLIM)
    ...
    ax.axvspan(
        CLINICAL_DECISION_WINDOW[0], CLINICAL_DECISION_WINDOW[1],
        color=RESPONSE_PALETTE["CR/PR"], alpha=0.08, label="Clinical Decision Window (0.20-0.50)"
    )
...
def plot_dca_curves(df_dca: pd.DataFrame, out_path: Path) -> None:
    ...
    strategies = [
        "Phenotype-Stratified (Q5)", "Global Predictor (Q1)",
        "CD274 (PD-L1+)", "High TMB", "Treat All", "Treat None",
    ]
    styles = {
    ...
    for strat in strategies:
        ...
        ax.plot(
            sub["Threshold"], sub["Net_Benefit"], label=strat,
            color=color, linestyle=st["linestyle"], linewidth=st["linewidth"],
        )

    _style_dca_axes(ax, float(df_dca["Net_Benefit"].max()))
    plt.tight_layout()
...
def _annotate_bars(ax: plt.Axes, fmt: str = ".2f") -> None:
    """Annotate bar heights on Seaborn barplot patches."""
    for p in ax.patches:
        h = p.get_height()
        if not np.isnan(h) and abs(h) > 0.0001:
            ax.annotate(
                f"{h:{fmt}}",
...
def _plot_ppv_panel(ax1: plt.Axes, df_sub: pd.DataFrame) -> None:
    ...
    sns.barplot(
        data=df_sub, x="Threshold_Label", y="PPV", hue="Strategy",
        palette=STRATEGY_PALETTE, ax=ax1, edgecolor="black", linewidth=0.8,
    )
    ...
    _annotate_bars(ax1, fmt=".2f")


def _plot_nnt_panel(ax2: plt.Axes, df_sub: pd.DataFrame) -> None:
    """Render Number Needed to Treat (NNT) panel."""
    sns.barplot(
        data=df_sub, x="Threshold_Label", y="NNT", hue="Strategy",
        palette=STRATEGY_PALETTE, ax=ax2, edgecolor="black", linewidth=0.8,
    )
...
def plot_unnecessary_treatments_avoided(df_dca: pd.DataFrame, out_path: Path) -> None:
    ...
    for strat in strategies:
        ...
        color = STRATEGY_PALETTE.get(strat, DARK_SLATE_CHARCOAL)
        ax.plot(
            sub["Threshold"], sub["Unnecessary_Treatments_Avoided"],
            label=strat, color=color, linewidth=2.5,
        )
...
def _build_phenotype_net_benefit_df(
    df: pd.DataFrame, prob_cols: Dict[str, np.ndarray], pt: float
) -> pd.DataFrame:
    """Build per-phenotype Net Benefit evaluation DataFrame at decision threshold pt."""
    ...
    for p_id, p_name in cluster_id_to_name.items():
        ...
        for strat_name in ["Phenotype-Stratified (Q5)", "Global Predictor (Q1)", "CD274 (PD-L1+)", "High TMB", "Treat All"]:
            y_prob_sub = np.ones(len(df_sub)) if strat_name == "Treat All" else prob_cols[strat_name][mask]
            nb, tp, fp, tn, fn = compute_net_benefit(y_true_sub, y_prob_sub, pt)
            rows.append({"Phenotype": p_name, "Strategy": strat_name, "Net_Benefit": nb, "N": len(df_sub)})
...
def plot_net_benefit_by_phenotype(
    ...
    df_pheno = _build_phenotype_net_benefit_df(df, prob_cols, pt)
    fig, ax = plt.subplots(figsize=(12, 6))
    sns.barplot(
        data=df_pheno, x="Phenotype", y="Net_Benefit", hue="Strategy",
...
def _load_valid_patient_data(input_file: Path) -> pd.DataFrame:
    """Load patient clusters and filter for known response status."""
...
def _export_utility_plots(
    df_valid: pd.DataFrame, df_dca: pd.DataFrame, prob_cols: Dict[str, np.ndarray]
) -> None:
    """Generate and save all Phase 6 clinical utility evaluation plots."""
    ...
    plot_net_benefit_by_phenotype(df_valid, prob_cols, FIXED_PT_THRESHOLD, pheno_plot_file)
...
def main() -> None:
    ...
    df_valid = _load_valid_patient_data(INPUT_FILE)
    prob_cols = generate_predictions(df_valid)

    df_dca = calculate_dca_curves(df_valid, prob_cols, DEFAULT_DCA_THRESHOLDS)
    dca_csv_path = OUTPUT_DIR / "dca_results.csv"
    safe_save_csv(df_dca, dca_csv_path)
    print(f"Saved DCA results to {rel_path(dca_csv_path)}")

    _export_utility_plots(df_valid, df_dca, prob_cols)
