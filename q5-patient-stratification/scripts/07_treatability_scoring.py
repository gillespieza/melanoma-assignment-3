...
def _arm_conf(df_assigned: pd.DataFrame, arm_prefix: str) -> Tuple[int, int, int]:
    """Per-arm confidence band breakdown counts."""
...
def _extract_treatability_markdown_metrics(df_assigned: pd.DataFrame) -> Dict[str, object]:
    """Extract metrics needed for Phase 7 markdown report rendering."""
...
def generate_phase7_markdown(df_assigned: pd.DataFrame, out_path: Path) -> None:
    ...
    m = _extract_treatability_markdown_metrics(df_assigned)
    ...
    doc_lines = [
        ...
        f"> - **What is being done**: Constructing a 3-arm clinical decision framework that routes all $N = {m['n_total']}$ patients into optimal therapeutic strategies: **Arm A** (Immunotherapy Monotherapy), **Arm B** (Targeted Therapy integrating Q2 Dabrafenib sensitivity model), and **Arm C** (Combination/Reversal Therapy integrating Q4 DepMap essentiality targets `CSF1R`, `MDM2`, `AXL`).",
        ...
        "",
        f"Phase 7 operationalises precision patient allocation across $N = {m['n_total']}$ patients. The decision engine routes patients into three structured therapeutic arms:",
        "",
        f"1. **Arm A: Immunotherapy Monotherapy** ($N = {m['n_arma']}$, **{m['pct_arma']:.1f}%** of cohort): Assigned to high-confidence responders (*Immune Hot* phenotype or high TIS scores). Received anti-PD-1 monotherapy (*Pembrolizumab* / *Nivolumab*).",
        f"2. **Arm B: Targeted Therapy (Q2 Integration)** ($N = {m['n_armb']}$, **{m['pct_armb']:.1f}%** of cohort): Assigned to predicted non-responders carrying actionable driver mutations (`BRAF V600` or `NRAS`). Integrates the Q2 LASSO cell viability regression model to compute a patient-specific **Dabrafenib Sensitivity Index** (mean Arm B sensitivity = **{m['mean_q2_dab']:.1f}/100**).",
        f"3. **Arm C: Combination & Microenvironmental Reversal (Q4 Integration)** ($N = {m['n_armc']}$, **{m['pct_armc']:.1f}%** of cohort): Assigned to remaining non-responders in immunologically cold or immunosuppressive microenvironments. Integrates Q4 DepMap essentiality targets to nominate helper interventions (most frequent nomination: **{m['top_target']}** with $N = {m['top_target_n']}$ patients).",
        "",
        ...
        "",
        f"- **Overall Mean Treatability Index**: **{m['mean_treat_overall']:.1f} / 100**",
        f"- **Immune Hot**: **{m['treat_hot']:.1f} / 100** (highest baseline sensitivity)",
        f"- **Mutant-Driven**: **{m['treat_mut']:.1f} / 100** (moderate convertibility via MAPK inhibition)",
        f"- **M2 Immunosuppressive**: **{m['treat_m2']:.1f} / 100** (convertible via `CSF1R` macrophage depletion)",
        f"- **Immune Cold**: **{m['treat_cold']:.1f} / 100** (lowest baseline; requires `AXL` / STING priming)",
        "",
        ...
        "",
        "#### Core Achievements",
        f"1. **Complete Decision Routing**: Successfully routed $N = {m['n_total']}$ patients into Arm A (**{m['pct_arma']:.1f}%**), Arm B (**{m['pct_armb']:.1f}%**), and Arm C (**{m['pct_armc']:.1f}%**).",
        ...
        f"3. **Mechanistic Reversal Nominations**: Identified `CSF1R` macrophage depletion as the primary helper target for *M2 Immunosuppressive* non-responders ($N = {m['top_target_n']}$ candidates).",
    ...
    if "Confidence_Band" in df_assigned.columns and "Recommendation_Confidence_Index" in df_assigned.columns:
        ...
        arma_h, arma_m, arma_l = _arm_conf(df_assigned, "Arm A")
        armb_h, armb_m, armb_l = _arm_conf(df_assigned, "Arm B")
        armc_h, armc_m, armc_l = _arm_conf(df_assigned, "Arm C")
        ...
        doc_lines += [
            ...
            "",
            f"Arm A results ($N = {m['n_arma']}$): High = {arma_h}, Moderate = {arma_m}, Low = {arma_l}. "
            ...
            "",
            f"Arm B results ($N = {m['n_armb']}$): High = {armb_h}, Moderate = {armb_m}, Low = {armb_l}. "
            ...
            "",
            f"Arm C results ($N = {m['n_armc']}$): High = {armc_h}, Moderate = {armc_m}, Low = {armc_l}. "
            ...
            "",
            f"#### Cohort-Level Confidence Summary ($N = {m['n_total']}$)",
            "",
            f"| Confidence Band | Count | Percentage |",
            f"|---|---|---|",
            f"| **High** | {n_high} | {n_high / m['n_total'] * 100:.1f}% |",
            f"| **Moderate** | {n_mod} | {n_mod / m['n_total'] * 100:.1f}% |",
            f"| **Low** | {n_low} | {n_low / m['n_total'] * 100:.1f}% |",
