...
1. **Arm A: Immunotherapy Monotherapy** ($N = 294$, **42.1%** of cohort): Assigned to high-confidence responders (*Immune Hot* phenotype or high TIS scores). Received anti-PD-1 monotherapy (*Pembrolizumab* / *Nivolumab*).
...
- **Immune Hot**: **50.1 / 100** (highest baseline sensitivity)
- **Mutant-Driven**: **53.8 / 100** (moderate convertibility via MAPK inhibition)
- **M2 Immunosuppressive**: **54.9 / 100** (convertible via `CSF1R` macrophage depletion)
...
3. **Mechanistic Reversal Nominations**: Identified `CSF1R` macrophage depletion as the primary helper target for *M2 Immunosuppressive* non-responders ($N = 51$ candidates).
...
Arm A results ($N = 294$): High = 36, Moderate = 126, Low = 132. The high proportion of Low-confidence Arm A patients reflects cases admitted via the borderline high-TIS rule rather than a clean Immune Hot phenotype — these are the patients most worth reviewing in a multidisciplinary team setting.
...
> **NRAS-only cap**: `NRAS`-only Arm B patients ($N = 103$) are **capped at Moderate** confidence regardless of their weighted score. The Q2 model was trained on Dabrafenib — a `BRAF`-directed drug — so its sensitivity predictions are less directly applicable to pure `NRAS` mutants, whose optimal inhibitor remains MEK or CDK4/6 combination therapy rather than Dabrafenib monotherapy.

Arm B results ($N = 288$): High = 141, Moderate = 143, Low = 4. Arm B achieves the highest proportion of High-confidence assignments across all three arms, reflecting that a clear oncogenic driver mutation paired with a strong Q2 drug sensitivity score is the most unambiguous routing signal in the system.
