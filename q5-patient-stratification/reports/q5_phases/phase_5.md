...
Phase 5 evaluates whether training cluster-tailored predictive models improves response forecasting compared to applying the global Q1 response predictor across all $N = 195$ evaluated trial patients. In the *Mutant-Driven* phenotype ($N = 100$), the subgroup-specific classifier achieved an ROC-AUC of 0.526 (compared to 0.534 for the global model). In the *Immunosuppressive M2-High* subset ($N = 73$), subgroup-specific modelling dramatically increased sensitivity and recall (40.0% vs 26.7%) and Positive Predictive Value (PPV = 48.0% vs 47.1%).
...
> - **Mutant-Driven** (orange, $N = 100$): Subgroup AUC = 0.526 vs Global AUC = 0.534 ($\Delta$ = -0.008, decline).
> - **Immune Cold** (blue, $N = 16$): Subgroup AUC = 0.400 vs Global AUC = 0.255 ($\Delta$ = +0.145, improvement).
> - **Immune Hot** (vermillion, $N = 6$): Subgroup AUC = 0.222 vs Global AUC = 0.556 ($\Delta$ = -0.333, decline).
> - **Immunosuppressive M2-High** (reddish purple, $N = 73$): Subgroup AUC = 0.567 vs Global AUC = 0.505 ($\Delta$ = +0.062, improvement).
...
> - **Highest ROC-AUC**: The *Immunosuppressive M2-High* subgroup model achieves the highest discriminative performance (AUC = 0.567), benefiting from the largest sample size and clearest driver mutation signal.
> - **Largest Recall Gain**: In the *Immune Cold* subgroup, phenotype-specific training increases Recall from 36.4% to 54.5% ($\Delta$ = +18.2 percentage points), identifying more true responders who would otherwise be missed by the global model.
