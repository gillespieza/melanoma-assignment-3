...
Phase 6 quantifies real-world clinical utility across $N = 195$ patients (82 objective responders, 42.1% baseline response rate) using the decision curve net benefit formulation:
...
1. **Superior Net Clinical Benefit**: At a standard decision threshold of $p_t = 0.30$, the Q5 Phenotype-Stratified decision system achieves a Net Benefit of **0.346**, outperforming empirical 'Treat All' (**0.172**), global Q1 prediction (**0.337**), and single-gene `CD274` (PD-L1+) biomarker selection (**0.165**).
2. **Number Needed to Treat (NNT) Reduction**: The Q5 decision model reduces the NNT to achieve one objective clinical response to **1.41** at $p_t = 0.30$, compared to an empirical NNT of **2.38** under 'Treat All' (an improvement of 40.5%).
3. **Toxicity Avoidance & Precision**: At $p_t = 0.30$, the Q5 system achieves a Positive Predictive Value (PPV) of **70.7%** (vs **42.1%** for 'Treat All') and successfully spares **79** non-responding patients from ineffective monotherapy toxicities.
4. **Robustness across Decision Thresholds**: Across all realistic clinical decision ranges ($p_t = 0.20 – 0.50$), the phenotype-stratified model maintains positive net benefit advantage over unstratified empirical treatment (Net Benefit at $p_t = 0.50$: Q5 = **0.379** vs Treat All = **-0.159**).
...
1. **Net Clinical Gain**: At a standard decision threshold of $p_t = 0.30$, the Q5 decision framework achieves a Net Benefit of **0.346**, outperforming empirical 'Treat All' (**0.172**) and single-gene `CD274` selection (**0.165**).
2. **Therapeutic Efficiency**: Reduces the Number Needed to Treat (NNT) to achieve one objective response to **1.41** at $p_t = 0.30$, compared to **2.38** for 'Treat All' — representing a **40.5%** reduction in futile treatment exposure.
3. **Toxicity Sparing & Safety**: Successfully identifies and spares **79** predicted non-responders from futile anti-PD-1 monotherapy, protecting patients from severe immune-related adverse events (irAEs) with no loss of treatment efficacy.
