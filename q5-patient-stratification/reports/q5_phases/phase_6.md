...
At a decision threshold of $p_t = 0.30$, the Q5 Phenotype-Stratified system achieves a Net Benefit of **0.303**, outperforming empirical 'Treat All' (**0.184**), global Q1 prediction (**0.293**), and single-gene `CD274` (PD-L1+) biomarker selection (**0.172**). The Number Needed to Treat (NNT) is reduced to **1.68** versus **2.38** under 'Treat All' (an improvement of 29.2%), sparing **57** non-responders from unnecessary toxicity.
...
> - **Key Takeaways**:
>   - **Superior Clinical Efficiency**: At $p_t = 0.30$, the Q5 Phenotype-Stratified system reduces NNT to **1.68** (vs **2.38** for Treat All), achieving a **29.2\% improvement** in treatment efficiency.
>   - **Enhanced Precision**: The Q5 system increases PPV to **59.4\%** (vs **42.1\%** for Treat All), ensuring a higher proportion of treated patients derive true clinical benefit.
...
1. **Net Clinical Gain**: At a standard decision threshold of $p_t = 0.30$, the Q5 decision framework achieves a Net Benefit of **0.303**, outperforming empirical 'Treat All' (**0.184**) and single-gene `CD274` selection (**0.172**).
2. **Therapeutic Efficiency**: Reduces the Number Needed to Treat (NNT) to achieve one objective response from **2.38** to **1.68** at $p_t = 0.30$, representing a **29.2%** reduction in futile treatment exposure.
3. **Toxicity Sparing & Safety**: Successfully identifies and spares **57** predicted non-responders from futile anti-PD-1 monotherapy, protecting patients from severe immune-related adverse events (irAEs) with no loss of treatment efficacy.
