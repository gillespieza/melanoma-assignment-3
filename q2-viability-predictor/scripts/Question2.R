...
# ============================================================
# STEP 13: Scatter plot of Q2 viability vs Q1 response
# ============================================================

merged_results <- read.csv("q2_correlation_results.csv", stringsAsFactors = FALSE)
cat("Rows loaded:", nrow(merged_results), "\n")

dir.create("plots", showWarnings = FALSE)

png("plots/q2_vs_q1_correlation.png", width = 1700, height = 1400, res = 200)
par(mar = c(5, 5, 5, 2))

plot(merged_results$PREDICTED_RESPONSE_SCORE,
     merged_results$Dabrafenib_Predicted_AUC,
     pch = 21, bg = "#2c7bb6", col = "grey30", cex = 1.6, lwd = 1.1,
     xlab = "Q1 predicted immunotherapy response",
     ylab = "Q2 predicted viability score (relative scale)",
     main = paste0("Q2 viability vs Q1 response\nr = -0.178  (n = ",
                   nrow(merged_results), " patients)"),
     cex.lab = 1.1, cex.main = 1.2)

abline(lm(Dabrafenib_Predicted_AUC ~ PREDICTED_RESPONSE_SCORE,
          data = merged_results),
       col = "#d7191c", lwd = 2, lty = 2)

dev.off()
cat("Saved plot to plots/q2_vs_q1_correlation.png\n")
